...
export default function TripDetailScreen() {
	...
	const badge = STATUS_BADGES[trip.status] ?? { label: trip.status, color: '#6B7280' };
	...
	return (
		<SafeAreaView className="flex-1" style={{ backgroundColor: themeColors.background }}>
			...
			<ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
				{/* Status Badge */}
				<Animated.View entering={FadeInDown.duration(400)} className="flex-row items-center justify-between mt-4 mb-4">
					<View className="flex-row items-center px-3 py-1.5 rounded-lg" style={{ backgroundColor: badge.color + '20' }}>
						<View className="w-2 h-2 rounded-full mr-2" style={{ backgroundColor: badge.color }} />
						<Text className="text-sm font-bold" style={{ color: badge.color }}>{badge.label}</Text>
					</View>
				...
				{/* Map */}
				<Animated.View entering={FadeInDown.duration(400).delay(100)} className="h-48 rounded-2xl overflow-hidden mb-4">
					<MapView
						style={{ flex: 1 }}
						initialRegion={{
				...
				>
					<View className="flex-row items-start">
						...
						<View className="flex-1 gap-2">
							<View>
								<Text className="text-sm font-bold" style={{ color: themeColors.text }}>{trip.pickupAddress}</Text>
								{trip.pickupReference && <Text className="text-xs text-gray-500">Ref: {trip.pickupReference}</Text>}
							</View>
							<View>
								<Text className="text-sm font-bold" style={{ color: themeColors.text }}>{trip.dropoffAddress}</Text>
								{trip.dropoffReference && <Text className="text-xs text-gray-500">Ref: {trip.dropoffReference}</Text>}
							</View>
						</View>
					</View>
				</Animated.View>
				...
				{driver && (
					...
					>
						<Text className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Motorista</Text>
				...
				>
					<Text className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Detalhes do Preço</Text>
					<PriceRow label="Subtotal" value={Number(trip.subtotal).toLocaleString('pt-AO')} />
					{trip.discountAmount > 0 && (
						<PriceRow label="Desconto" value={`-${Number(trip.discountAmount).toLocaleString('pt-AO')} Kz`} color="#10B981" />
					)}
					<PriceRow label="IVA (14%)" value={`${Number(trip.ivaAmount).toLocaleString('pt-AO')} Kz`} />
					<PriceRow label="Taxa de serviço" value={`${Number(trip.serviceFee).toLocaleString('pt-AO')} Kz`} />
					{trip.surgeMultiplierApplied > 1 && (
						<PriceRow label="Tarifa dinâmica" value={`${trip.surgeMultiplierApplied}x`} />
					)}
					<View className="border-t mt-2 pt-2 flex-row justify-between"
						style={{ borderColor: isDark ? '#333' : '#E5E7EB' }}
					>
						<Text className="text-base font-bold" style={{ color: themeColors.text }}>Total</Text>
					...
					<View className="flex-row justify-between mt-1">
						...
						<Text className="text-xs font-bold text-gray-500">{trip.paymentMethod}</Text>
					</View>
				</Animated.View>
				...
				>
					<View className="flex-1 p-4 rounded-2xl border items-center"
						style={{
							backgroundColor: isDark ? '#1A1A1A' : '#F9FAFB',
							borderColor: isDark ? '#333' : '#E5E7EB',
						}}
					>
						<Ionicons name="speedometer-outline" size={24} color={themeColors.primary} />
						<Text className="text-lg font-black mt-1" style={{ color: themeColors.text }}>
				...
				{events && events.length > 0 && (
					<Animated.View entering={FadeInDown.duration(400).delay(350)}
						className="mb-6"
					>
						<Text className="text-base font-bold mb-4" style={{ color: themeColors.text }}>
							Linha do Tempo
						</Text>
				...
				{trip.status === 'COMPLETED' && !showDispute && (
					<Animated.View entering={FadeInUp.duration(400).delay(400)} className="mb-6">
						<TouchableOpacity
							onPress={() => setShowDispute(true)}
							className="py-3 rounded-2xl items-center border border-red-500/20 bg-red-500/10"
						>
							<Text className="text-base font-bold text-red-500">Abrir Disputa</Text>
						</TouchableOpacity>
					</Animated.View>
				)}
				...
				{showDispute && (
					...
					>
						<Text className="text-base font-bold mb-3" style={{ color: themeColors.text }}>Abrir Disputa</Text>
...
function PriceRow({ label, value, color }: { label: string; value: string; color?: string }) {
	...
	return (
		<View className="flex-row justify-between mb-1.5">
			<Text className="text-sm" style={{ color: color ?? (isDark ? '#CCC' : '#666') }}>{label}</Text>
			<Text className="text-sm font-semibold" style={{ color: color ?? themeColors.text }}>{value}</Text>
		</View>
	);
}
	...
	return (
		<View className="flex-row">
			<View className="w-6 items-center">
				...
				{!isLast && <View className="w-0.5 flex-1 bg-gray-300 dark:bg-gray-700 my-1" />}
			</View>
			<View className={`flex-1 ml-3 ${isLast ? '' : 'mb-4'}`}>
				<Text className="text-sm font-semibold" style={{ color: themeColors.text }}>{label}</Text>
				<Text className="text-xs text-gray-500">
					{new Date(event.createdAt).toLocaleTimeString('pt-AO', { hour: '2-digit', minute: '2-digit' })}
				</Text>
			</View>
		</View>
	);
